...
function distributeByShortestColumn(
  ...
  const columns: MasonryGalleryImage[][] = Array.from({ length: columnCount }, () => [])
...
/**
 * Full-bleed responsive masonry gallery with shuffle + lightbox.
 */
export function MasonryGallery({ images }: MasonryGalleryProps) {
  ...
  const [shuffled] = useState(() => fisherYatesShuffle([...images]))
