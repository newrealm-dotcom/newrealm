import { Link } from 'react-router-dom'
import type { Project } from '../data/projects'
import { assetUrl } from '../lib/assetUrl'

export function ProjectCard({ project }: { project: Project }) {
  return (
    <Link
      to={`/work/${project.slug}`}
      className="group relative flex h-full flex-col rounded-[20px] border border-white/25 bg-[#00419f] transition-colors hover:border-white/60"
    >
      <div className="flex items-start justify-between px-7 pt-7">
        <span className="eyebrow !text-white">{project.tag}</span>
        <span className="text-white/60 transition-transform group-hover:translate-x-1 group-hover:-translate-y-1">
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            <path d="M5 13L13 5M13 5H6M13 5V12" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </span>
      </div>

      {project.cover ? (
        <div className="mt-6 px-7">
          <div className="relative overflow-hidden rounded-[20px]">
            <img
              src={assetUrl(project.cover)}
              alt=""
              className="aspect-[16/9] w-full object-cover"
            />
            <span
              aria-hidden="true"
              className="pointer-events-none absolute inset-0 rounded-[20px] shadow-[inset_0_0_0_5px_#fff]"
            />
          </div>
        </div>
      ) : null}

      <div className="px-7 pb-7 pt-6">
        <p className="eyebrow mb-2 !text-white">{project.industry}</p>
        <h3 className="font-[family-name:var(--font-display)] text-2xl font-medium text-white">
          {project.client}
        </h3>
        <p className="mt-2 text-sm text-white/75">{project.summary}</p>
      </div>
    </Link>
  )
}
